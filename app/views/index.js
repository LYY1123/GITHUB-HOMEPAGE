...
window.devFlag = window.location.origin.indexOf('10') > 0 || window.location.origin.indexOf('localhost') || window.location.origin.indexOf('dev') ? 1 : 0;
...
// 注册service worker
registerServiceWorker().then(function(flag){
  vm.offlineState = "done";
})

setTimeout(function () {
  // 检测访问浏览器
  if (window.navigator && window.navigator.userAgent) {
    if (window.navigator.userAgent.indexOf('Mobile') >= 0 && window.navigator.userAgent.indexOf('Android') >= 0 && window.navigator.userAgent.indexOf('Chrome') >= 0) {
      vm.pwa = "done";
    }
  }
},5000)
